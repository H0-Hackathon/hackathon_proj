...
const HandoffQueue = () => {
  ...
  const fetchHandoffs = async () => {
    try {
      ...
      antMessage.error('Failed to load handoff queue');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusTag = (status) => {
    const config = {
      ...
      processing: { color: 'blue', text: 'In Progress', icon: <ClockCircleOutlined /> },
  ...
  const getReasonText = (reason) => {
    const reasons = {
      ...
      manual_request: 'Manual Transfer',
          ...
          <Space size="large">
            <div>
              ...
              <div style={{ fontSize: 12, color: '#8c8c8c', marginTop: 4 }}>
                Pending: {pendingCount} | In Progress: {processingCount}
              </div>
            </div>
            <Space>
              <Button
